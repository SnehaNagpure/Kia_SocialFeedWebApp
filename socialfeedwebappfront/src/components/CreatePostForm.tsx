...
const CreatePostForm = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFiles = Array.from(e.target.files);
    const combinedFiles = [...formData.media, ...newFiles];
  ...
  return (
    <form className="form-container" onSubmit={handleSubmit}>
      ...
      <label>Upload Media (up to 3 images & 1 video):</label>
      <input
        type="file"
        ref={fileInputRef}
        accept="image/*,video/*"
        ...
        <div className="preview-container">
          {formData.media.map((file, i) => (
            <div key={i} className="preview-box">
              {file.type.startsWith('video/') ? (
                <video src={previews[i]} width={120} height={90} controls />
              ) : (
                <img
                  src={previews[i]}
                  alt={`preview-${i}`}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '1rem' }}>
        <button type="button" onClick={() => navigate('/Profile')} disabled={submitting}>
          Cancel
        </button>
